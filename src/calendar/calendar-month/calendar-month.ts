...
export class CalendarMonthComponent implements OnInit {
  ...
  private initViews(date: Moment) {
    ...
    // 获取1号的星期下标
    const startWeek = this.startWeekDay === 'Monday' ? currentDate.set({ 'date': 1 }).isoWeekday() - 1 :
    ...
    for (let i = 0; i < startWeek; i++) {
      // 上一个月
      const date = lastDate.set({ date: lastDate.daysInMonth() - i }).format('YYYY-MM-DD');
    ...
    // 获取最后一天星期下标
    const endWeek = this.startWeekDay === 'Monday' ? currentDate.set({ 'date': days }).isoWeekday() - 1 :
    ...
    if (this.dates.length < 42) {
      ...
      if (endDate.format('YYYY-MM-DD') === currentDate.format('YYYY-MM-DD')) {
        ...
        for (let i = 0; i < emptyDay; i++) {
          ...
          const date = endDate.set({ date: endDay + i + 1 }).format('YYYY-MM-DD');
          this.dates.push(date);
